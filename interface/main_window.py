...
class MainWindow(QMainWindow):
	...
	def init_main_window(self):
		...
		self.media_player.video_widget.frame_clicked.connect(self.update_coordinates)
		...
		final_layout.addWidget(self.event_window)

		# Set stretch factors for the widgets
		final_layout.setStretch(0, 1)  # ListDisplay (first widget)
		final_layout.setStretch(1, 3)  # VideoDisplay (second widget) - wider
		final_layout.setStretch(2, 1)  # EventSelectionWindow (third widget)
	...
	def keyPressEvent(self, event):
		...
		if event.key() == Qt.Key_Delete or event.key() == Qt.Key_Backspace:
			...
			if index >= 0:
				...
				self.list_display.display_list(self.list_manager.create_text_list())
				path_label = self.media_player.path_label
